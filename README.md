# mightygraphics Validation - PHP Standalone Validation Library

[![Build Status](https://img.shields.io/travis/mightygraphics/validation.svg?style=flat-square)](https://travis-ci.org/mightygraphics/validation)
[![Coverage Status](https://coveralls.io/repos/github/mightygraphics/validation/badge.svg?branch=setup_coveralls)](https://coveralls.io/github/mightygraphics/validation)
...
```
composer require "mightygraphics/validation"
```
...
Errors messages are collected in `mightygraphics\Validation\ErrorBag` object that you can get it using `errors()` method.
...
<details><summary><strong>alpha_dmightygraphics</strong></summary>

The field under this rule may have alpha-numeric characters, as well as dmightygraphicses and underscores.
...
> Note: `mightygraphics\Validation\Rules\Callback` instance is binded into your Closure.
...
Another way to use custom validation rule is to create a class extending `mightygraphics\Validation\Rule`.
...
In `UniqueRule` above, property `$message` is used for default invalid message. And property `$fillable_params` is used for `fillParameters` method (defined in `mightygraphics\Validation\Rule` class). By default `fillParameters` will fill parameters listed in `$fillable_params`. For example `unique:users,email,exception@mail.com` in example above, will set:
...
To do this, you should implements `mightygraphics\Validation\Rules\Interfaces\ModifyValue` and create method `modifyValue($value)` to your custom rule class.
...
To do this, you should implements `mightygraphics\Validation\Rules\Interfaces\BeforeValidate` and create method `beforeValidate()` to your custom rule class.
...
class YourCustomRule extends Rule implements BeforeValidate
{
    ...
    {
        $attribute = $this->getAttribute(); // mightygraphics\Validation\Attribute instance
        $validation = $this->validation; // mightygraphics\Validation\Validation instance
